...
import { closeCycleInGitHub } from "@/lib/close-cycle.mjs";
...
export async function POST(request: Request) {
  ...
  if (!user) return json({ error: "AUTH_REQUIRED", signIn: "/signin-with-chatgpt?return_to=%2F" }, 401);
  const runtime = process.env as RuntimeEnv;
  if (!runtime.EP_OPERATOR_EMAIL || user.email.toLowerCase() !== runtime.EP_OPERATOR_EMAIL.toLowerCase()) return json({ error: "FORBIDDEN" }, 403);
  if (!runtime.EP_GITHUB_TOKEN) return json({ error: "INTEGRATION_NOT_CONFIGURED" }, 503);
  ...
  try {
    return json({ ok: true, ...await closeCycleInGitHub({ owner: "valerol", repo: "ep_dashboard", branch: "main", token: runtime.EP_GITHUB_TOKEN, input, actor: user.email }) });
  } catch (error) {
